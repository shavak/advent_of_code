...
fn part_a(input_path: &Path) {
    ...
    if let Ok(lines) = read_lines(input_path) {
        ...
        v = split_whitespace_parse::<u64>(&lines_f.next().unwrap());
        while let Some(line) = lines_f.next() {
            if line == "" {
                ...
                let u: Vec<u64> = split_whitespace_parse(&line);
...
fn part_b(input_path: &Path) {
    ...
    if let Ok(lines) = read_lines(input_path) {
        let mut lines_f = lines.flatten();
        let h: Vec<u64> = split_whitespace_parse::<u64>(&lines_f.next().unwrap());
        let m = h.len();
        let mut i = 0_usize;
        while i < m {
            v.push((h[i], h[i + 1]));
        ...
        while let Some(line) = lines_f.next() {
            if line == "" {
                ...
                let w: Vec<u64> = split_whitespace_parse(&line);
